...
from rag.prompts.generator import chunks_format, citation_prompt, cross_languages, full_question, full_question_kr, kb_prompt, keyword_extraction, message_fit_in, \
...
async def async_chat(dialog, messages, stream=True, **kwargs):
    ...
    if len(questions) > 1 and prompt_config.get("refine_multiturn"):
        questions = [await full_question_kr(dialog.tenant_id, dialog.llm_id, messages)]
        # full_question_kr() 생성된 question을 디버깅 로그로 출력
        logging.info(f">>> Full question generated by full_question_kr(): {questions[0]}")
    else:
